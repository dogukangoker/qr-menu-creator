...
const ListProduct = () => {
    const [openDeleteDialog,
        setopenDeleteDialog] = useState(false);
    const [productId,
        setProductId] = useState('');
    const [dialogText,
        setDialogText] = useState("")
    const [editable,
        setEditable] = useState(false);
    const [menu,
        setMenu] = useState([]);
    const [category,
    ...
    const deleteProduct = async() => {
        ...
        if (dialogText === "Onaylıyorum") {
            try {
                let data = await axios.post("http://localhost:5000/product/delete/" + productId);
                console.log(data);
                setEditable(true);
                getCategoryData();
                setDialogText("");
    ...
    const updateProduct = async() => {
        ...
        if (!productId) return;
        if(!product_name || !product_price || !product_description || !product_category || !product_image) {
        ...
        try{
            let data = await axios.post("http://localhost:5000/product/update/" + productId, {
                product_name,
                product_price,
                product_description,
                product_category,
    ...
    <div className={style.container}>
        ...
        <h5>Kategorize ederek ürünleri listelemek için aşağıdaki bölümden kategori seçiniz.</h5>
        <Select
            style={{width: '100%'}}
          ...
          {category.map((item, index) => {
            return <MenuItem key={index} value={item.category_name}>{item.category_name}</MenuItem>
            })}
        ...
        <table className={style.table}>
            ...
            <tbody>
                   {listCategory ? currentProducts.filter(item => item.product_category === listCategory).map((item, index) => {
                    return (
                        <tr key={index}>
                            ...
                            <td>
                                <Button onClick={() => 
                                    ...
                                    }} variant="contained" color="primary" startIcon={< EditIcon />}>Düzenle</Button>
                            </td>
                            <td
                                onClick={() => {
                                ...
                                <Button variant="contained" color="secondary" startIcon={< DeleteIcon />}>Sil</Button>
                            </td>
                        </tr>
                    )                
                    ...
                    return (
                        <tr key={index}>
                            ...
                            <td>{item.product_description}</td>
                            <td>{item.product_price}</td>
                            <td><img src={item.product_image} alt="product_image" width="100" height="100"/></td>
                            <td>
                                <Button onClick={() => 
                                    {
                                        setProductName(item.product_name);
        ...
        <Pagination productsPerPage={productsPerPage} totalProducts={menu.length} paginate={paginate} />
        <DeleteProduct
