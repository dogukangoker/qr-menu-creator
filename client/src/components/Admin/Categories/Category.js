...
const Category = () => {
    ...
    const [openAddDialog,
        setAddOpenDialog] = useState(false);
    const [error,
        setError] = useState("");
    const [openEditDialog,
        setEditOpenDialog] = useState(false);
    const [categoryName,
    ...
    const [categorySlug,
        setCategorySlug] = useState("");
    const [openDeleteDialog,
        setOpenDeleteDialog] = useState(false);
    const [currentPage,
        setCurrentPage] = useState(1);
    const [productsPerPage,
        setProductsPerPage] = useState(5);
    ...
    const deleteCategory = () => {
        ...
        if (dialogText == "Onaylıyorum") {
            try {
                axios.post("http://localhost:5000/category/delete/" + categoryId);
    ...
    const addNewCategory = async() => {
        ...
        if (categoryName == "" || categoryImage == "") {
            ...
            try {
                let data = await axios.post("http://localhost:5000/category/addcategory", {
    ...
    const editCategory = async() => {
        ...
        if (categoryName == "" || categoryImage == "") {
            ...
            try {
                let data = await axios.post("http://localhost:5000/category/update/" + categoryId, {
                    category_name: categoryName,
                    category_image: categoryImage,
    ...
    const currentCategories = category.slice(indexOfFirstProduct, indexOfLastProduct);
    // sayfa degistirme
    ...
    return (
        <div className={style.container}>
            <div className={style.header}>
                <h1>KATEGORİLER</h1>
                <Button  className={style.addButton} color="success" onClick={handleClickOpenDialog} variant="contained"><AddCircleIcon/>
                    <span
                        style={{
                        marginLeft: '10px'
                    }}>YENİ KATEGORİ EKLE</span>
                </Button>
            </div>
