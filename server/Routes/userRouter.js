...
router.post("/signup", async (req, res) => {
  try {
    ...
    Logger.info(`${username} kullanıcısı oluşturuldu.`);
...
router.post("/signin", async (req, res) => {
  try {
    ...
    Logger.info(`${username} kullanıcısı giriş yaptı.`);
...
router.get("/listuser", async (req, res) => {
  try {
    ...
    res.status(200).json({ users, message: "Kullanıcılar listelendi." });
  } catch (e) {
    res.status(400).json({ message: e.message });
  }
});
...
router.post("/update/:id", async (req, res) => {
  try {
    ...
    Logger.info(`${username} kullanıcısı güncellendi.`);
...
router.post("/delete/:id", async (req, res) => {
  try {
    ...
    Logger.info(`${user.username} kullanıcısı silindi.`);
